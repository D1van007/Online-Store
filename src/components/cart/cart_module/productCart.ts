...
export class ProductInCart {
    ...
    constructor(dataProduct: IProduct, selectorList: HTMLUListElement, index: number) {
        ...
        this.productAmount = JSON.parse(localStorage.getItem(`productAmount-id${dataProduct.id}`)!)
        this.renderProduct()
        this.productItemID = document.getElementById(this.dataProduct.id.toString())!
        this.amountContentDOM = document.getElementById(`amount-change_content${dataProduct.id}`)!
        this.amountProductDOM = document.getElementById(`amount-product${dataProduct.id}`)!
        this.totalPriceProductDOM = document.getElementById(`product_item__total-price${dataProduct.id}`)!
        this.changeAmount()
    ...
    renderProductNum() {
        let numList = document.querySelectorAll('.number__item')
    ...
    changeAmount() {
        this.productItemID.addEventListener('click', (event) => {
            ...
            if (JSON.parse(localStorage.getItem('products_inCart')!).length === 0 || !JSON.parse(localStorage.getItem('products_inCart')!)) {
                this.clearCart()
            }
...
function createHTMLCartItem(dataProduct: IProduct, index: number, amountValue: number) {
    ...
    <div>
        ...
        <span id="product_item__total-price${dataProduct.id}" class="product_item__total-price">Total: €${amountValue * dataProduct.price}</span>       
    </div>    
</li>`
}
