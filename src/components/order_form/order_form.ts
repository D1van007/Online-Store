...
export class OrderForm{
  ...
  submitButtonEventHeandler(){
    let allInputs = this.formContainerDOM.querySelectorAll('input')
  ...
  eMailValidation(){
    ...
    let currentValue = this.eMailDOM.value
  ...
  changeCardImage(){
    if(this.cardNumberDOM.value.length>0&&this.cardNumberDOM.value.length<2){
      let children = this.cardLogoContainerDOM.querySelectorAll('i')
