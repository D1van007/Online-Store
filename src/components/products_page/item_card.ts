...
function renderHTML(id:number,data:IProduct):string{
  return `
  <li class="product-item" id = "${id}" style = "background-image: url(${data.images[0]});">
    <h3 class="product-item__title">${data.title}</h3>
    <div class="product-item__price">$${data.price}</div>
    <div class="product-item__price">/q${data.stock}</div>
